...
class DetectionConfig:
    """Tunable detection parameters. Pass to MCQDetector / AdvancedQuestionDetector."""
    # Scoring thresholds
    margin_score: int = 50
    margin_penalty: int = 100
    number_pattern_score: int = 40
    option_penalty: int = 50
    min_score: int = 60
    # Visual MCQ detection
    mcq_kernel_w: int = 100
    mcq_kernel_h: int = 40
    mcq_dilate_iterations: int = 4
    mcq_min_w: int = 200
    mcq_min_h: int = 100
    mcq_max_h_frac: float = 0.4
    # Standard visual detection
    std_kernel_w: int = 50
    std_kernel_h: int = 30
    std_dilate_iterations: int = 3
    std_min_w: int = 100
    std_min_h: int = 50


def merge_overlapping_boxes(boxes: list[dict], iou_threshold: float = 0.3) -> list[dict]:
    """
...
class MCQDetector:
    ...
    def __init__(self, doc, config: DetectionConfig | None = None):
        self.doc = doc
        self.det_cfg = config or DetectionConfig()
    ...
    def _detect_mcq_visual(self, page):
        ...
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (self.det_cfg.mcq_kernel_w, self.det_cfg.mcq_kernel_h))
        dilated = cv2.dilate(thresh, kernel, iterations=self.det_cfg.mcq_dilate_iterations)
        ...
        for cnt in contours:
            ...
            if cw > self.det_cfg.mcq_min_w and ch > self.det_cfg.mcq_min_h and ch < h * self.det_cfg.mcq_max_h_frac:
                boxes.append({
...
class AdvancedQuestionDetector:
    ...
    def detect_questions_on_page(self, page_index: int) -> list[dict]:
        if not self.doc: return []
        page = self.doc[page_index]
        blocks = page.get_text("blocks")
        ...
        for b in blocks:
            ...
            score = 0
            if abs(x - primary_margin) <= 15: score += self.det_cfg.margin_score
            elif x > primary_margin + 20: score -= self.det_cfg.margin_penalty

            if re.match(r"^\d+\.", text): score += self.det_cfg.number_pattern_score
            elif re.match(r"^Q(uestion)?\s*\d+", text, re.IGNORECASE): score += self.det_cfg.number_pattern_score

            if re.match(r"^\d+\)", text): score -= self.det_cfg.option_penalty
            if re.match(r"^\(\d+\)", text): score -= self.det_cfg.option_penalty
            if re.match(r"^[A-E]\.", text): score -= self.det_cfg.margin_penalty

            if len(text) < 4 and not re.match(r"^\d+\.$", text): score -= 20

            if score >= self.det_cfg.min_score:
    ...
    def _detect_by_image_fallback(self, page):
        ...
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (self.det_cfg.std_kernel_w, self.det_cfg.std_kernel_h))
        dilated = cv2.dilate(thresh, kernel, iterations=self.det_cfg.std_dilate_iterations)
        contours, _ = cv2.findContours(dilated, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        ...
        for cnt in contours:
            ...
            if cw > self.det_cfg.std_min_w and ch > self.det_cfg.std_min_h:
                boxes.append({
